...
int main(int mama, char **moo)
{
  ...
  if (SSL_CTX_use_certificate_file(ctx, "../ca/intermediate/certs/boromail.cert.pem", SSL_FILETYPE_PEM) != 1)
  {
    ERR_print_errors_fp(stderr);
    exit(EXIT_FAILURE);
  }

  if (SSL_CTX_use_PrivateKey_file(ctx, "../ca/intermediate/private/boromail.key.pem", SSL_FILETYPE_PEM) != 1)
  {
    ERR_print_errors_fp(stderr);
    exit(EXIT_FAILURE);
  }

  if (SSL_CTX_load_verify_locations(ctx, "../ca/intermediate/certs/intermediate.cert.pem", NULL) != 1) {
    ERR_print_errors_fp(stderr);
    exit(EXIT_FAILURE);
  }
